...
export default function PreviewSlider({ item }) {
  ...
  useEffect(() => {
    preview.current = document.querySelector('.main-container div')
  ...
  function mapMainPreview() {
    ...
    for (let i = 0; i < image_links.length; i++) {
      result.push(
        <div className='flex-row justify-center align-center hidden' id={`img${i}`} key={i}>
  ...
  function showToMain(e) {
    ...
    preview.current = document.querySelector(`.main-container #${e.target.alt}`)
    preview.current.classList.remove('hidden')
  }
